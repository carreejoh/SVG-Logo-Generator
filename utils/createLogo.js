...
module.exports = {
    ...
    generateSVG(data) {
        if(data.textchar.length >= 4) {
            ...
            return `<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="300" height="200"> \n
            '<polygon points="40,200 260,200 150,0" fill="${data.shapecolor}" />' \n
            <text x="150" y="180" font-size="60" text-anchor="middle" fill="${data.textcolor}">${data.textchar}</text>
            </svg>`;
        } else if (data.shapedesign === "Circle" && data.textchar.length <= 3) {
            return `<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="300" height="200"> \n
            '<circle cx="150" cy="100" r="80" fill="${data.shapecolor}"></circle>' \n
            <text x="150" y="125" font-size="60" text-anchor="middle" fill="${data.textcolor}">${data.textchar}</text>
            </svg>`;
